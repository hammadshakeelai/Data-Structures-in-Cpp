...
public:
    ...
    void Add_Student(string n, int id) {
        ...
        if (head == nullptr) {// if empty
            head = student;// add node at head directly
            return;
        }
        Student* curr = head;//make a temp variable
        while (curr->next != nullptr) {// loop till the last node : As( LIFO )
            curr = curr->next;// next next
        }
        curr->next = student;// add at end
    }
    ...
    if (position < 0) {//position invalid
        cout << "Invalid position." << endl;
        return false;
    }
   
    Student* temp = head;// goto the end
